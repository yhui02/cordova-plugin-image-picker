...
- (void)deselectAsset:(PHAsset *)asset
{
    ...
    if(self.selectedAssets.count > 0 && self.selectedAssets.count < (_maximumImagesCount + 1))
        [self updateDoneButton];
...
- (void)updateDoneButton
{
    ...
    for (UIViewController *viewController in nav.viewControllers)
        viewController.navigationItem.rightBarButtonItem.enabled = (self.selectedAssets.count > 0 && self.selectedAssets.count < (_maximumImagesCount + 1));
}
...
- (NSString *)toolbarTitle
{
    ...
    else if (nImages > _maximumImagesCount){
      return [NSString stringWithFormat:NSLocalizedStringFromTable(@"picker.select.limit", @"GMImagePicker", @"最多可选择 %@ 张图片"), @(_maximumImagesCount)];
    }
    else if (nImages>1)
    {
